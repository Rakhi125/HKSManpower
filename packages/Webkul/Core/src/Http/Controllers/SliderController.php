...
//$path = $request->file('filename1')->store('storage/app/public/slider_images/Default/'.$request->filename1);
$name = $request->file('filename1')->getClientOriginalName();
 $imageName = time().'.'.$request->filename1->extension();  
     
 $request->filename1->move(public_path('img'), $imageName);

 DB::table('sliders')->insert([
    'title' => $request->heading1,
    'path' => public_path('img').$imageName,
    'alt_text' => $request->text1,
    'slider_path' =>$request->url1,
    'channel_id' =>'1',
    'locale' =>'en'
]);
