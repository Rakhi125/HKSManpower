...
@php
    ...
    // $isSellerProduct = app('Webkul\Marketplace\Repositories\ProductRepository')->findOnewhere(['product_id'=>$product->id]);
    $isSellerProduct = app('Webkul\Marketplace\Repositories\ProductRepository')->getMarketplaceProductByProduct($product->id, auth()->guard('customer')->user()->id);
...
@push('scripts')
    ...
    <script>
        ...
        @else
            var downloadableLinks = @json([]);
            var downloadableSamples = @json([]);
        @endif
